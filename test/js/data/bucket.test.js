...
test('Bucket', function(t) {
    ...
    function createClass(options) {
        ...
        Class.prototype.addFeature = function(feature) {
            ...
            group.layout.secondElement.emplaceBack(point.x, point.y);
    ...
    t.test('add features', function(t) {
        ...
        var testVertex = bucket.arrays.test[0].layout.vertex;
        t.equal(testVertex.length, 1);
        var v0 = testVertex.get(0);
        t.equal(v0.box0, 34);
        t.equal(v0.box1, 84);
        var paintVertex = bucket.arrays.test[0].paint.layerid;
        t.equal(paintVertex.length, 1);
        var p0 = paintVertex.get(0);
        t.equal(p0.map, 17);

        var testElement = bucket.arrays.test[0].layout.element;
        ...
        var testSecondElement = bucket.arrays.test[0].layout.secondElement;
    ...
    t.test('add features, multiple layers', function(t) {
        ...
        var v0 = bucket.arrays.test[0].layout.vertex.get(0);
        var a0 = bucket.arrays.test[0].paint.one.get(0);
        var b0 = bucket.arrays.test[0].paint.two.get(0);
        t.equal(a0.map, 17);
        t.equal(b0.map, 17);
        t.equal(v0.box0, 34);
        t.equal(v0.box1, 84);
    ...
    t.test('add features, disabled attribute', function(t) {
        ...
        t.equal(bucket.arrays.test[0].layout.vertex.bytesPerElement, 0);
        t.deepEqual(
    ...
    t.test('reset buffers', function(t) {
        ...
        t.equal(bucket.arrays.test.length, 1);
        bucket.createArrays();
        t.equal(bucket.arrays.test.length, 0);
    ...
    t.test('add features after resetting buffers', function(t) {
        ...
        t.equal(v0.box0, 34);
        t.equal(v0.box1, 84);
        var testPaintVertex = bucket.arrays.test[0].paint.layerid;

...
function drawLayerSymbols(painter, source, layer, coords, isText,
    ...
    for (var j = 0; j < coords.length; j++) {
        ...
        if (!bucket) continue;
        var bothBufferGroups = bucket.buffers;
        ...
        drawSymbol(painter, layer, coords[j].posMatrix, tile, bucket, bufferGroups, isText,
                isText || metadata.sdfIcons, !isText && metadata.iconsNeedLinear,
                isText ? metadata.adjustedTextSize : metadata.adjustedIconSize, metadata.fontstack,
                translate,
                translateAnchor,
                rotationAlignment,
                size,

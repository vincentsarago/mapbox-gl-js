...
function drawFill(painter, source, layer, coord) {
    ...
    if (!bucket) return;
    var bufferGroups = bucket.buffers.fill;
...
function drawStroke(painter, source, layer, coord) {
    ...
    for (var k = 0; k < bufferGroups.length; k++) {
        ...
        group.secondVaos[layer.id].bind(gl, program, group.layout.vertex, group.layout.secondElement);
        gl.drawElements(gl.LINES, group.layout.secondElement.length * 2, gl.UNSIGNED_SHORT, 0);
    }
}
